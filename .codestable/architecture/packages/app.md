...
| 内容数据库 | 只读 schema 5 SQLite、类型化事实/关系/条件/视觉、卡片/筛选、别名和 FTS 查询 | `core/database/content/ContentDatabaseFactory.kt`、`ContentDatabase.kt`、`ContentDatabaseManager.kt` |
...
- 页面只消费 `player-facts-v1` 类型化 DTO；列表/搜索使用经一致性校验的卡片与 facet 投影，详情按需读取规范事实和关系，不把 raw JSON 当回退事实源。
- 图片路径必须在活动包根目录内；官方无图和代理视觉与包缺图/损坏分开。未知事实和无法解析关系不制造假实体。
