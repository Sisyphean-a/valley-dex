...
本文面向把本项目构建产物内嵌到手机端 Wiki 的开发者。它描述当前 `schema_version = 2` 的数据包、SQLite 表、稳定 JSON 字段、查询方式和升级规则。
...
1. 解压整个包到应用数据目录的临时位置。
2. 读取 `manifest.json`，要求 `format = "stardew-offline-data"`，并只接受应用已支持的 `schemaVersion`。
3. 计算 `database.sha256`，必须等于 `stardew.db` 的 SHA-256；再执行 `PRAGMA quick_check`，结果必须为 `ok`。
4. 读取 `build_meta.schema_version`，它必须与 manifest 的 `schemaVersion` 一致。
...
| 表 | 用途 | 写入关系 | 移动端建议 |
|---|---|---|---|
| `build_meta` | 数据库级元数据 | 每次构建写 7 项 | 打开库时读取版本与语言。 |
...
| `name_zh` | `TEXT`，非空 | 主展示名；没有官方中文时会退回英文或空字符串。 |
...
| `translation_status` | `TEXT`，非空 | `complete`：有官方中文名；`missing`：应翻译但缺失；`not_applicable`：技术记录等无需翻译。 |
| `image_path` | `TEXT`，可空 | 包根目录相对的 WebP 路径。文件不存在时应显示占位图，不要从 `imageSource` 现场裁图。 |
...
所有类型都可能有：`_provenance.official`（参与生成的官方文件路径数组）。映像相关补充字段为 `imageSource`（原始 PNG 来源）、`spriteIndex`（图集格索引）、`imageFallbackSources`（村民备用图路径）和 `imageMode`（村民为 `full`）；它们仅用于构建追溯，不是移动端图片入口。
...
当 `schemaVersion` 增大时，应用必须拒绝启用该包，直到完成数据库适配。相同 schema 下即使 `builderVersion` 或 `gameVersion` 改变，也应以字段存在性读取 JSON：只展示认识的字段，忽略未知字段，绝不因未知字段而覆盖或删除原始数据。
