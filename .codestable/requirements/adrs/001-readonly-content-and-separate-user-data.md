...
`stardew.db` 是外部生成、随数据包整体替换的内容库；收藏、历史和笔记属于应用自己的用户状态。把两类数据写进同一库会让内容更新、回滚和用户数据迁移互相耦合。

## 决定

内容库用 Android `SQLiteDatabase` 只读打开，并由 `ContentDatabaseManager` 独占句柄；收藏、历史、笔记和最近搜索使用独立的 Room `user.db`。用户表中的实体 ID 是软引用，内容包更新不自动删除记录。

## 真实备选

1. 用 Room 接管外部内容库并随包做模式迁移。
2. 把收藏、历史和笔记直接写入 `stardew.db`。
...
- 收藏、历史和笔记需要在 UI 层处理当前包缺失实体，但不会因实体删除而静默丢失。
