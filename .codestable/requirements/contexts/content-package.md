...
- `DataPackageValidator` 在 staging 包上校验数据库/清单/报告内容绑定、SQLite `quick_check` 与 `foreign_key_check`、schema 指纹、必需索引、元数据一致性、核心事实槽、状态和值组合、条件完整性、关系目标/方向/证据、补充事实状态、搜索/筛选投影及视觉清单。上述结构正确性零容错；任何坏包拒绝并保留原活动包。日常内容库仍以只读方式打开。
