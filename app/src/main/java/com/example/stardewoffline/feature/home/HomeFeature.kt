...
@HiltViewModel
class HomeViewModel @Inject constructor(private val content: ContentRepository) : ViewModel() {
    private val mutableTypes = MutableStateFlow<List<EntityTypeCount>>(emptyList())
    val types = mutableTypes.asStateFlow()
    init { viewModelScope.launch { (content.typeCounts() as? AppResult.Success)?.let { mutableTypes.value = it.value } } }
}

@Composable
fun HomeRoute(onType: (String) -> Unit, viewModel: HomeViewModel = hiltViewModel()) {
    ...
    LazyColumn(Modifier.fillMaxSize(), contentPadding = PaddingValues(16.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
        ...
        items(types.filter { it.count > 0 }, key = { it.type }) { type ->
